...
Zkusme nyni pridat koeficienct $a$, tedy budeme zobrazovat prubeh funkce $y = ax^2$. Z prednasky cislo 11 vime, ze koeficienct $a$ urcuje **sirku paraboly**  a **orientaci paraboly**. Cim vice se koeficienct $a$ blizi 0, tim je parabola sirsi. Pokud je koeficient $a$ **kladny**, tak ma parabola tvar pismena "U". V tomto pripade ma kvadraticka funkce *globalni minimum*, ktere urcuje ypsilonova souradnice vrcholu paraboly. Kvadraticka funkce s kladnym koeficientem $a$ *nema maximum* (ramena paraboly totiz rostou do plus nekonecna). Pokud je koeficient $a$ **zaporny**, tak ma parabola tvar prevraceneo pismena "U". V tomto pripade ma kvadraticka funkce *globalni maximum*, ktere urcuje ypsilonova souradnice vrcholu paraboly. Kvadraticka funkce se zapornym koeficientem $a$ *nema minimum* (ramena paraboly totiz klesaji do minus nekonecna). 
Pokud v ramci legendy grafu formulujeme argument `bty = "n"`, tak se legenda do grafu doplni bez ramecku.
```{r}
plot(x, type = "n", 
     ylim = c(1, 200), 
     xlim = c(-10, 10),
...
# a = 1.5 (vysledkem bude uzsi parabola)
curve(1.5 * x^2, 
      from = x[1], to = x[n], 
      add = TRUE, 
      lwd = 2, 
...
# a = 0.5 (vysledkem bude sirsi parabola)
curve(0.5 * x^2, 
      from = x[1], to = x[n], 
      add = TRUE, 
      lwd = 2, 
...
legend("topleft", 
       legend = as.expression(c(bquote(y == 1.5*x^2), bquote(y == x^2), bquote(y == 0.5*x^2))), 
...
Ted zkusme pridat konstantu $c = 10$ a prubeh kvadraticke funkce si opet zobrazit do grafu. Budeme tedy zobrazovat prubeh funkce $y = ax^2 + c$. Stejne jako u linearni funkce tato konstanta znaci, kde krivka protina vertikalni osu y. Jelikoz jsme si pro tento priklad zvolili konstantu 10, tak modra parabola protina vertikalni osu grafu v bode [0; 10].
```{r}
plot(x, type = "n", 
     ylim = c(1, 200), 
     xlim = c(-10, 10),
...
# c = 10 (parabola se posune podel vertikalni osy o 10 smerem nahoru)
curve(1.5 * x^2 + 10, 
      from = x[1], to = x[n], 
      add = TRUE, 
      lwd = 2,
...
legend("topleft", 
       legend = as.expression(c(bquote(y == x^2), bquote(y == 1.5*x^2), bquote(y == 1.5*x^2 + 10))), 
       col = c("black", "red", "blue"), 
       lwd = c(2, 2, 2),
       bty = "n")
```

Nakonec pridame linearni clen $bx$, kde koeficient $b$ bude rovno 3. Budeme tedy zobrazovat prubeh kvadraticke funkce $y = ax^2 + bx + c$.
```{r}
plot(x, type = "n", 
     ylim = c(1, 200), 
     xlim = c(-10, 10),
...
legend("topleft", legend = bquote(y == 2^x), lwd = 2)
```
...
D(expression(2^x), "x")
# logaritmicka funkce - prirozeny logaritmus
D(expression(log(x)), "x")
# logaritmicka funkce - logaritmus se zakladem 2
D(expression(log(x)/log(2)), "x")
```
...
```{r}
# funkce nasobena konstantou
D(expression(10*x^5), "x")
# soucet dvou funkci
D(expression((x^4) + (3*x^-10)), "x")
# soucin dvou funkci
D(expression((x^4) * (3*x^10)), "x")
# podil dvou funkci
...
Na uplny zaver shrnme tyto informace o prubehu funkce do grafu. Vyuzijeme k tomu funkci `rect`, ktera do grafu nakresli barevne odlisene obdelniky podle toho, jestli funkce na nejakem intervalu definicniho oboru roste nebo klesa. U funkce `rect` je potreba definovat souradnice kazde strany obdelniku (argumenty: `xleft`, `ybottom`, `xright` a `ytop`). Take je s vyuzitim funkce `text` mozne do grafu na libovolne misto doplnit jakykoli text. U funkce `text` je potreba definovat x-ovou a y-ovou souradnici, kde ma byt text do grafu doplnen (argumenty: `x` a `y`).
```{r}
x <- seq(from = -4, to = 4, by = 1)
y <- -x^4 + 8*x^2 - 7
plot(x, y, type = "n", ylim = c(-100, 100), xaxp = c(-4, 4, 8), yaxp = c(-100, 100, 10), cex = 2, las = 1, ylab = "y", main = bquote(y == -x^4 + 8*x^2 - 7))
...
abline(v = 0)
abline(h = 0)
rect(xleft = -5, ybottom = -120, xright = -2, ytop = 120, col = adjustcolor("green", alpha.f = 0.1))
rect(xleft = -2, ybottom = -120, xright = 0, ytop = 120, col = adjustcolor("red", alpha.f = 0.1))
rect(xleft = 0, ybottom = -120, xright = 2, ytop = 120, col = adjustcolor("green", alpha.f = 0.1))
rect(xleft = 2, ybottom = -120, xright = 5, ytop = 120, col = adjustcolor("red", alpha.f = 0.1))
text(x = -3, y = 80, "rostouci", cex = 1.5)
text(x = -1, y = 80, "klesajici", cex = 1.5)
text(x = 1, y = 80, "rostouci", cex = 1.5)
text(x = 3, y = 80, "klesajici", cex = 1.5)
```
Z grafu polynomu ctvrteho stupne je take vidno, ze se jedna o **sudou funkci**. To znamena, ze plati: $f(x) = f(-x)$. Graf funkce je tedy soumerny podle vertikalni osy *y*.
